...
<template>
  <div class="card">
    ...
    <div class="p-6">
      <!-- Sensor Cards Grid -->
      <div v-if="sensorCount > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <div
          v-for="(sensor, sensorId) in sensorData"
          :key="sensorId"
          class="group relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 hover:border-gray-300 dark:hover:border-gray-600 hover:shadow-md transition-all duration-200 cursor-pointer"
        ...
        >
          ...
          <div class="flex items-start gap-4 mb-6">
            <div
              :class="`w-12 h-12 bg-${getSensorColor(sensorId)}-100 dark:bg-${getSensorColor(sensorId)}-900/30 rounded-lg flex items-center justify-center flex-shrink-0`"
            >
              <span
          ...
          <div class="flex items-center justify-between">
            ...
            >
              <div class="w-1.5 h-1.5 rounded-full" :class="getStatusInfo(sensor.status).dot"></div>
